...
docker-compose exec postgres sh -c "pg_dump -h 35.188.201.2 -U postgres -d dojot_auth -O -c -Fc> /tmp/dojot_auth.sql"
echo "Postgres: dojot_auth database has been dumped"
docker-compose exec postgres sh -c "pg_dump -h 35.188.201.2 -U postgres -d dojot_devm -O -c -Fc > /tmp/dojot_devm.sql"
echo "Postgres: dojot_devm database has been dumped"
docker-compose exec postgres sh -c "pg_dump -h 35.188.201.2 -U postgres -d dojot_imgm -O -c -Fc> /tmp/dojot_imgm.sql"
